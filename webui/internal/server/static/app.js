...
if (systemMonitorOpen && systemMonitorDialog) {
  ...
  const renderMonitor = (data) => {
    ...
    renderTable("[data-monitor-diskio]", Array.isArray(data.diskio) ? data.diskio : [], [
      {label:"Disk", render:(row)=>row.disk_name || row.name}, {label:"Read", render:(row)=>rate(row.read_bytes)}, {label:"Write", render:(row)=>rate(row.write_bytes)}
    ]);
    renderTable("[data-monitor-network]", Array.isArray(data.network) ? data.network : [], [
      {label:"Interface", render:(row)=>row.interface_name || row.name}, {label:"RX", render:(row)=>rate(row.rx)}, {label:"TX", render:(row)=>rate(row.tx)}
    ]);
