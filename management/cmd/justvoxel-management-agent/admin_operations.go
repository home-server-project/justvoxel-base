...
func registerAdminOperationRoutes(mux *http.ServeMux, s *server) {
	...
	mux.HandleFunc("GET /v1/admin/reset/minecraft/current-operation", s.adminCurrentMinecraftResetOperation)
	mux.HandleFunc("GET /v1/admin/reset/factory/current-operation", s.adminCurrentFactoryResetOperation)
...
func (s *server) adminCurrentMinecraftResetOperation(w http.ResponseWriter, r *http.Request) {
	if _, ok := s.requireAdministrator(w, r); !ok {
		return
	}
	if s.operations == nil {
	...
	operation, err := s.operations.currentMinecraftReset()
	if err != nil {
		writeError(w, http.StatusInternalServerError, "current Minecraft reset operation could not be read")
		return
	}
	writeJSON(w, http.StatusOK, adminOperationResponse{Operation: operation})
}

func (s *server) adminCurrentFactoryResetOperation(w http.ResponseWriter, r *http.Request) {
	if _, ok := s.requireAdministrator(w, r); !ok {
		return
	}
	if s.operations == nil {
	...
	operation, err := s.operations.currentFactoryReset()
	if err != nil {
		writeError(w, http.StatusInternalServerError, "current factory reset operation could not be read")
		return
	}
	writeJSON(w, http.StatusOK, adminOperationResponse{Operation: operation})
}
