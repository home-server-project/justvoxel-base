...
\n'/ }"
    value="$(sed -E         -e 's/(password|passwd)=[^,[:space:]]+/\1=[redacted]/Ig'         -e 's/credentials=[^,[:space:]]+/credentials=[redacted]/Ig'         <<< "${value}" 2>/dev/null || printf '%s' "${value}")"
    value="$(tr -s '[:space:]' ' ' <<< "${value}" 2>/dev/null || printf '%s' "${value}")"
    value="${value#" "}"
    value="${value%" "}"
    if [[ -z ${value} ]]; then
